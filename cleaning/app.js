...
/** date -> [{itemId,itemName,person,memo,histIndex}], newest history entries last-in per item preserved by index */
function recordsByDate(){
  ...
  DATA.forEach(function(d){
    ...
    s.history.forEach(function(h, idx){
      ...
      map[h.date].push({itemId:d.id, itemName:d.name, person:h.person, memo:h.memo, histIndex:idx});
    });
  });
  return map;
}
...
function recordLine(h){
  ...
  return esc(fmtISOJP(h.date))+' <b>'+who+'</b>'+(h.memo?(' — '+esc(h.memo)):'');
}
...
function renderCalendar(today){
  ...
  var cellsHtml = cells.map(function(d){
    ...
    var dot = recList.length ? '<span class="cal-dot">'+recList.length+'</span>' : "";
    return '<button type="button" class="'+cls+'" data-date="'+ds+'"><span class="cal-daynum">'+d.getDate()+'</span>'+dot+'</button>';
  }).join("");
  return (
...
function renderSummary(today){
  var items = allMerged();
  var counts = {overdue:0, due:0, unlogged:0, scheduled:0};
  items.forEach(function(it){
    var s = displayStatus(it, today).status;
    if(counts[s]!=null) counts[s]++;
  });
  return (
    '<div class="summary-row">'+
      '<span class="sumchip od">期限超過 <b>'+counts.overdue+'</b></span>'+
      '<span class="sumchip du">清掃時期 <b>'+counts.due+'</b></span>'+
      '<span class="sumchip ul">未記録 <b>'+counts.unlogged+'</b></span>'+
      '<span class="sumchip ok">順調 <b>'+counts.scheduled+'</b></span>'+
    '</div>'
  );
}
...
var root, dlgLog, dlgEdit, dlgSettings, dlgDay, toastEl;
var curDayDate = null;
...
function paint(){
  var today = todayDate();
  var body;
  if(activeTab==="calendar") body = '<div id="calendar-tab">'+renderCalendar(today)+'</div>';
...
function buildDialogs(){
  ...
  dlgLog.innerHTML =
    '<div class="dlg-body">'+
      ...
      '<div class="dlg-actions">'+
        ...
        '<button class="btn primary" id="log-submit" type="button">記録する</button>'+
      '</div>'+
    '</div>';
  ...
  dlgDay.innerHTML =
    '<div class="dlg-body">'+
      ...
      '<div class="dlg-actions">'+
        '<button class="btn" data-close="1" type="button">閉じる</button>'+
        '<button class="btn primary" id="day-submit" type="button">追加する</button>'+
      '</div>'+
    '</div>';
  document.body.appendChild(dlgDay);

  [dlgLog, dlgEdit, dlgSettings, dlgDay].forEach(function(d){
    d.querySelectorAll('[data-close]').forEach(function(b){
      b.addEventListener('click', function(){ dlgHide(dlgLog); dlgHide(dlgEdit); dlgHide(dlgSettings); dlgHide(dlgDay); });
    });
  });
...
function refreshDayList(){
  ...
  wrap.innerHTML = '<ul class="day-rec-list">'+recs.map(function(r){
    return '<li><b>'+esc(r.itemName)+'</b><span>担当: '+esc(r.person||"不明")+(r.memo?(' ・ '+esc(r.memo)):'')+'</span>'+
...
function submitLog(){
  ...
  mutateAndSave(function(state){
    ...
    s.history = s.history || [];
    s.history.push({date:date, person:person, memo: memo || undefined});
    recomputeLastDone(s);
  }, "清掃完了記録: " + DATA_BY_ID[id].name + " (" + date + " " + person + ")");
}
function submitEdit(){
  if(!curEditId) return;
...
function submitDayAdd(){
  ...
  mutateAndSave(function(state){
    ...
    s.history.push({date:date, person:person, memo: memo || undefined});
    recomputeLastDone(s);
  }, "清掃完了記録: " + DATA_BY_ID[itemId].name + " (" + date + " " + person + ")", function(){
    dlgDay.querySelector('#day-memo').value = "";
  });
}
...
function init(){
  ...
  buildDialogs();
  dlgLog.querySelector('#log-submit').addEventListener('click', submitLog);
  ...
  dlgDay.querySelector('#day-submit').addEventListener('click', submitDayAdd);
